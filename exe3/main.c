...
void btn_callback(uint gpio, uint32_t events) {
    if (events == GPIO_IRQ_EDGE_FALL) {
        if (gpio == BTN_PIN_R) {
            xSemaphoreGiveFromISR(xSemaphoreR, NULL);
        } else if (gpio == BTN_PIN_G) {
            xSemaphoreGiveFromISR(xSemaphoreG, NULL);
        }
    }
}
...
void led_2_task(void *p) {
    ...
    int delay = 0;
    while (true) {
        xQueueReceive(xQueueButId2, &delay, 0);
...
void btn_2_task(void *p) {
    ...
    int delay = 0;
    while (true) {
        if (xSemaphoreTake(xSemaphoreG, portMAX_DELAY) == pdTRUE) {
            vTaskDelay(pdMS_TO_TICKS(150));
...
int main() {
    ...
    xQueueButId2 = xQueueCreate(1, sizeof(int));
